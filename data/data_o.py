...
def collate_fn(batch): # add support for motion and object features
    ...
    inds = [item[5] for item in batch]
    multiclass_ind = torch.stack(inds,0)

    rels = [item[6] for item in batch]
    multiclass_rel = torch.stack(rels,0)
    
    return images,padVar,m,max_target_len,ides,motion_batch,object_batch,multiclass_ind,multiclass_rel

class CustomDataset(Dataset):
    
    def __init__(self,json_data,cfg,appearance_feature_dict, annotation_dict , video_name_list, voc,motion_feature_dict=None,
    ...
    def __getitem__(self,idx):
        ...
        if self.dataset == "test":
            multiclass_inds = None
            multiclass_rels = None
        else:
            dp = self.dataset[idx]
        ...
        return appearance_tensor,anno_index,self.v_name_list[idx],motion_tensor,object_tensor,multiclass_inds,multiclass_rels
...
class DataHandler:
    ...
    def _read_feature_file(self,feature_type='appearance'):
        ...
        if self.cfg.model_name == 'mean_pooling' or self.cfg.model_name == 'marn' :
            for key in f1.keys():
...
#         return vid2url
    ...
    def getDatasets(self,json_data):
        
        if self.cfg.model_name =='marn':
            train_dset = CustomDataset(json_data,self.cfg,self.appearance_feature_dict, self.train_dict, self.train_name_list, self.voc,
                                      self.motion_feature_dict,self.object_feature_dict)
            val_dset = CustomDataset(json_data,self.cfg,self.appearance_feature_dict, self.val_dict, self.val_name_list, self.voc,
                                    self.motion_feature_dict,self.object_feature_dict)
            test_dset = CustomDataset("test",self.cfg,self.appearance_feature_dict, self.test_dict, self.test_name_list, self.voc,
                                     self.motion_feature_dict,self.object_feature_dict)
    ...
    def getDataloader(self,train_dset,val_dset,test_dset):
        ...
        train_loader=DataLoader(train_dset,batch_size = self.cfg.batch_size, num_workers = 0,shuffle = True, collate_fn = collate_fn, drop_last=True)
        val_loader = DataLoader(val_dset,batch_size = self.cfg.batch_size, num_workers = 0,shuffle = False,collate_fn = collate_fn,
                         drop_last=False)
        test_loader = DataLoader(test_dset,batch_size = self.cfg.batch_size, num_workers = 0,shuffle = False,collate_fn = collate_fn,
                         drop_last=False)
        '''
        train_loader=DataLoader(train_dset,batch_size = 2, num_workers =0,shuffle = True,
                        collate_fn = collate_fn, drop_last=True)

        val_loader = DataLoader(val_dset,batch_size = 2, num_workers = 0,shuffle = False,collate_fn = collate_fn,
                         drop_last=False)
        test_loader = DataLoader(test_dset,batch_size = 2, num_workers = 0,shuffle = False,collate_fn = collate_fn,
                         drop_last=False)
        '''
